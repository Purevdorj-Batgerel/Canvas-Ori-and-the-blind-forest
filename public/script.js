window.onload = () => {
    ...
    function drawAnimation() {
        for (let i = 0; i < fliesCount; i++) {
            ...
            let distance = Math.sqrt((f.x - mouseX) * (f.x - mouseX) + (f.y - mouseY) * (f.y - mouseY));
            ...
            if (distance > 100) {
                ...
                console
                radGrad.addColorStop(0, 'rgba(255,255,164,' + (0.7 + (0.3 * (100 - distance) / 100)) + ')')
                radGrad.addColorStop(0.4, 'rgba(247,148,29,' + (0.3 + (0.4 * (100 - distance) / 100)) + ')')
                radGrad.addColorStop(1, 'rgba(255,218,164,0)')
            }
        ...
        for (let i = 0; i < fliesCount; i++) {
            ...
            if (p.x > window.innerWidth + p.r * 2 || p.x < -p.r * 2 || p.y < -p.r * 2) {
                if (i % 3 > 0) { //66.67% of the flakes
                    ...
                    if (xChange > 0) { //Enter from the left
                        fireFlies[i] = {
                            x: -p.r * 2 + 1,
                            y: Math.random() * window.innerHeight,
                            r: p.r,
